  ...
  <div class="teacher-manage">
    ...
    <el-card class="search-card" shadow="never">
      <el-form :inline="true" :model="searchForm">
        <el-form-item label="关键词">
          <el-input v-model="searchForm.keyword" placeholder="搜索姓名或学院" clearable
    ...
    <el-card class="table-card" shadow="never">
      ...
      <el-table :data="tableData" v-loading="loading" stripe border style="width: 100%">
        <el-table-column prop="teacherId" label="工号" width="90" align="center" />
        <el-table-column prop="teacherName" label="姓名" width="100" />
        <el-table-column prop="sex" label="性别" width="60" align="center" />
        <el-table-column prop="type" label="职称" width="100" />
        <el-table-column prop="institute" label="学院" min-width="140" show-overflow-tooltip />
        <el-table-column prop="tel" label="电话" width="130" />
        <el-table-column prop="email" label="邮箱" width="180" show-overflow-tooltip />
        <el-table-column label="操作" width="160" align="center" fixed="right">
          <template #default="{ row }">
            <el-button link type="primary" size="small" @click="openEdit(row)">编辑</el-button>
            <el-popconfirm title="确定删除该教师？" @confirm="handleDelete(row.teacherId)">
              <template #reference>
        ...
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item label="职称" prop="type">
              <el-input v-model="form.type" placeholder="如：讲师、副教授" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
import { listTeachers, createTeacher, updateTeacher, deleteTeacher, type Teacher } from '@/api/teacher'
...
async function fetchList() {
  loading.value = true
  try {
    const res = await listTeachers(searchForm.keyword, searchForm.page, searchForm.size)
...
const emptyForm = (): Teacher => ({
  teacherId: null, teacherName: '', institute: '', sex: '',
  tel: '', email: '', pwd: '', cardId: '', type: '', role: '1'
})
const form = reactive<Teacher>(emptyForm())
...
function openEdit(row: Teacher) {
  isEdit.value = true
  editId.value = row.teacherId
  Object.assign(form, { ...row, pwd: '' }) // 编辑时密码清空，留空不修改
  dialogVisible.value = true
}
